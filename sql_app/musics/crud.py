...
def create_music(db: Session, music: schemas.MusicCreate):
    db_music = models.Music(title=music.title, audio_file_url=music.audio_file_url,
                            cover_img_url=music.cover_img_url, genre=music.genre._value_, owner_id=music.owner_id)
    db.add(db_music)
    db.commit()
    db.refresh(db_music)
    print(db_music)
